...
test_in_raw = pd.read_csv('test_in.txt', header = None)
test_out_raw = pd.read_csv('test_out.txt', header = None)
...
for i in range(len(test_in)):
    ...
    just_zero.append(int(y_pred[0]))
    Y_pred.append(y_pred[0])
print(just_zero)
print("just : 1")
for i in range(len(test_out)):
    y_pred=knn.predict(test_out[[i]])
    just_one.append(int(y_pred[0]))
    Y_pred.append(y_pred[0])
print(just_one)
