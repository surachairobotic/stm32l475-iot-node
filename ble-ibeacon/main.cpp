...
public:
    BeaconDemo(BLE &ble, EventQueue &event_queue) :
        _ble(ble),
...
private:
    ...
    void start_advertising() {
        ...
        if (error) {
            pc.printf("Set advertise parameters error\n");
            return;
        }
        ...
        if (error) {
            pc.printf("Set advertise payload error\n");
            return;
        }
        ...
        if (error) {
            pc.printf("Start advertising error\n");
            return;
        }
    }
...
private:
    ...
    void onDisconnectionComplete(const ble::DisconnectionCompleteEvent&) {
        ...
        pc.printf("on disconnection event\n");
    }
...
{
    /*
    ble::vendor::cordio::CordioHCIDriver &driver = CordioHCIHook::get_driver();
    ble::vendor::cordio::CordioHCITransportDriver &transport_driver = CordioHCIHook::get_transport_driver();
    driver.initialize();
    ble::vendor::cordio::CordioHCIHook::set_data_received_handler(hci_driver_rx_reset_handler);
    */
